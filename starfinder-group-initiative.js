const sortArrayOfObjects = (arr, propertyName, order = 'ascending') => {
  ...
  const isInTurnOrder = function(id, turnorder) {
    var isInTurnOrder = false;
    ...
    return isInTurnOrder;
  };

  const overwriteTurnOrder = function(id, init, turnorder) {
    for (let i = 0; i < turnorder.length; i++) {
        if (turnorder[i].id === id) {
            turnorder[i].pr = init;
            break;
      ...
      if(userCommand === '!group-init') {
          ...
          var m_obj = _.chain(msg.selected)  // Start a chain of the selected objects
              ...
              .each(function(t){
                  ...
                  // replace if existing ID, don't add to array
                  if (isInTurnOrder(t.get('id'), turnorder)) {
                    turnorder = overwriteTurnOrder(t.get('id'), initiative, turnorder);
                  } else {
                    turnorder.push({
                        'id': t.get('id'),
                        'pr': initiative,
